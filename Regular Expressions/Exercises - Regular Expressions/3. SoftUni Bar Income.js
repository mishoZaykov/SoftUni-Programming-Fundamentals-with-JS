function softUniBarIncome(customerArray) {
  ...
  const pattern = /%(?<customer>[A-Z][a-z]+)%[^|$%.]*<(?<product>\w+)>[^|$%.]*\|(?<count>[0-9]+)\|[^0-9\|$%.]*(?<price>[0-9|\.]+)\$/gm;
  ...
  while (command !== 'end of shift') {
    ...
    if(currentData){
      ...
      let totaProductPrice = currentCount * currentPrice;

      console.log(`${currentCustomer}: ${currentProduct} - ${totaProductPrice.toFixed(2)}`);
      totalIncome += totaProductPrice;
